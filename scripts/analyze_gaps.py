...
def export_to_js(assessments: list[dict], week_statuses: dict[int, str], out_path: Path) -> None:
    """Write gap data as a JS file that the portal can load as window.CSA_GAP_DATA."""
    import json
    ...
    for d in assessments:
        ...
        domains_out.append({
            ...
            "evidence": d["evidence"],
            "phase": mapping.get("phase"),
            "weeks": mapping.get("weeks", []),
            "slot": mapping.get("slot"),
            "focus": mapping.get("focus", ""),
        })

    weeks_out = [
        {"week": n, "status": week_statuses.get(n, "Not Started")}
        for n in range(1, 11)
    ]
    ...
    js_content = (
        ...
        f"window.CSA_GAP_DATA = {json.dumps(payload, indent=2)};\n"
    )
    ...
    print(f"Gap data exported to: {display_path}")
    print("Open index.html in a browser to see the Gap Analysis section.")
...
def _parse_args() -> tuple[Path, Path | None]:
    ...
    i = 0
    while i < len(args):
        if args[i] == "--assessment" and i + 1 < len(args):
            assessment_path = Path(args[i + 1])
            i += 2
        elif args[i] == "--export":
            # optional next arg is custom output path; default is gap_report.js at repo root
            if i + 1 < len(args) and not args[i + 1].startswith("--"):
                ...
                export_path = ROOT / "gap_report.js"
                i += 1
        else:
            i += 1
    return assessment_path, export_path


def main() -> None:
    assessment_path, export_path = _parse_args()
    ...
    if export_path is not None:
        # Check for unfilled defaults before exporting
        if all(d["current"] == 1 and d["target"] == 5 for d in assessments):
            print("WARNING: All domains show placeholder values (1 → 5).")
            print("Fill docs/SKILL_ASSESSMENT_TEMPLATE.md with your actual levels first.")
            sys.exit(1)
        export_to_js(assessments, week_statuses, export_path)
    else:
